...
(function() {
   ...
   */
  function createFireAvatar(level = 'none', sizeKey = 'md', glow = true, pulse = true, ring = false) {
    const sizeMap = { xs: 16, sm: 20, md: 36, lg: 72 };
    const size = typeof sizeKey === 'number' ? sizeKey : (sizeMap[sizeKey] || sizeMap.md);
  ...
  function sizeFromProgressPercent(pct) {
    ...
    let px = PROGRESS_AVATAR_MIN + (PROGRESS_AVATAR_MAX - PROGRESS_AVATAR_MIN) * eased;
  ...
  function generateSampleAssignments() {
    ...
    return [
      { id: 'a1', title: 'JavaScript Quiz prep', points: 5,  dueDate: new Date(now.getTime() + dayMs * 2).toISOString(), completed: false },
      { id: 'a2', title: 'Module 2 reading',     points: 2,  dueDate: new Date(now.getTime() + dayMs * 3).toISOString(), completed: false },
      { id: 'a3', title: 'Discussion post',       points: 3,  dueDate: new Date(now.getTime() + dayMs * 4).toISOString(), completed: false },
      { id: 'a4', title: 'Project research',      points: 4,  dueDate: new Date(now.getTime() + dayMs * 7).toISOString(), completed: false }
    ];
  }
    ...
    return [
      {
        ...
        unread: 0,
        messages: [
          { sender: 'Prof. Smith', text: 'Don’t forget about the quiz tomorrow!', time: Date.now() - 86400000 },
          { sender: 'Me',          text: 'Thanks for the reminder!',            time: Date.now() - 80000000 }
        ]
      },
      {
        id: 'c2',
        ...
        unread: 2,
        messages: [
          { sender: 'Alice', text: 'Has anyone started the project yet?', time: Date.now() - 3600000 },
          { sender: 'Bob',   text: 'Not yet, maybe we can meet this weekend.', time: Date.now() - 1800000 },
          { sender: 'Me',    text: 'I can meet on Saturday afternoon.', time: Date.now() - 600000 }
        ]
      },
      {
        id: 'c3',
        ...
        unread: 1,
        messages: [
          { sender: 'Jane', text: 'Can you explain the last lecture to me?', time: Date.now() - 7200000 }
        ]
      }
    ];
  }
  ...
  function initStudentWidget() {
    ...
    let currentUnderstanding = localStorage.getItem(UNDERSTANDING_KEY) || 'none';
    // Load conversations
    ...
    understandingPrompt.textContent = 'What is your level of understanding? Please click an emoji.';
      ...
      tasks.forEach(task => {
        ...
        if (task.dueDate) {
          ...
          if (!isNaN(due.getTime())) {
            dueStr = due.toLocaleDateString(undefined, { month: 'short', day: 'numeric' });
          }
        }
        const meta = [];
        if (task.points !== undefined && task.points !== null) meta.push(`${task.points} pts`);
    ...
    function updateMatchSection() {
      ...
      if (currentUnderstanding === 'low') {
        ...
        targets = students.filter(s => s.understanding === 'medium' || s.understanding === 'high');
      } else if (currentUnderstanding === 'high') {
        // recommend low
        targets = students.filter(s => s.understanding === 'low');
      } else {
        matchSection.appendChild(document.createTextNode('Select your understanding level to see peers.'));
        return;
      }
      if (targets.length === 0) {
        matchSection.appendChild(document.createTextNode('No matching peers found.')); 
        return;
      }
    ...
    function openChannelModal(targetName) {
      ...
      function renderThreads() {
        ...
        conversations.forEach(thread => {
          ...
          preview.textContent = lastMsg ? `${lastMsg.sender}: ${lastMsg.text.slice(0, 30)}` : '';
          ...
          threadDiv.addEventListener('click', () => openConversation(thread.id));
      ...
      function openConversationByName(name) {
        ...
        if (!thread) {
          ...
          const id = 'c' + Date.now();
          thread = { id, title: name, participants: ['Me', name], unread: 0, messages: [] };
          conversations.push(thread);
          saveJSON(CONVOS_KEY, conversations);
        }
    ...
    function renderThreadList() {
      ...
      conversations.forEach(thread => {
        ...
        const lastMsg = thread.messages[thread.messages.length - 1];
        previewSpan.textContent = lastMsg ? `${lastMsg.sender}: ${lastMsg.text.slice(0, 30)}` : '';
        const unreadBadge = document.createElement('span');
        unreadBadge.className = 'inferno-unread-badge';
        if (thread.unread > 0) {
          unreadBadge.textContent = thread.unread;
        ...
        threadItem.addEventListener('click', () => openConversationInPanel(thread.id));
    ...
    if (currentUnderstanding && currentUnderstanding !== 'none') {
      chrome.runtime.sendMessage({ type: 'signal', signal: currentUnderstanding });
    }
  }
  ...
  function initInstructorPanel() {
    ...
    ['Broadcast','Export CSV'].forEach(itemText => {
      ...
      item.addEventListener('click', () => {
        ...
        if (itemText === 'Broadcast') {
          ...
          broadcastContainer.style.display = broadcastContainer.style.display === 'none' ? '' : 'none';
          ...
          students.forEach(s => {
            csvRows.push(`${s.name},${s.understanding},${s.grade},${s.tasksCompleted},${s.tasksTotal}`);
          });
    ...
    ['low','medium','high'].forEach(key => {
      ...
      labelSpan.textContent = key === 'low' ? 'Red' : key === 'medium' ? 'Blue' : 'Green';
    ...
    broadcastTextarea.placeholder = 'Write a message to send to multiple students...';
    ...
    ['Red','Blue','All'].forEach(label => {
      ...
      btn.addEventListener('click', () => {
        alert(`Broadcast sent to ${label} students: ${broadcastTextarea.value}`);
    ...
    function renderStudentsList() {
      ...
      let filtered = students.filter(s => {
        const matchFilter = activeFilters.length === 0 || activeFilters.includes(s.understanding);
      ...
      filtered.forEach(s => {
        ...
        const progressRatio = s.tasksTotal ? s.tasksCompleted / s.tasksTotal : 0;
        const progressPct = Math.round(progressRatio * 100);
        const avatarSizePx = sizeFromProgressPercent(progressPct);
        const avatarEl = createFireAvatar(s.understanding || 'none', avatarSizePx);
    ...
    function renderPairings() {
      ...
      const redSorted = redStudents.slice().sort((a,b) => progressRatio(b) - progressRatio(a));
      const blueSorted = blueStudents.slice().sort((a,b) => progressRatio(b) - progressRatio(a));
      const greenSorted = greenStudents.slice().sort((a,b) => progressRatio(b) - progressRatio(a));
    ...
    function renderThreadListInstructor() {
      ...
      conversations.forEach(thread => {
        ...
        item.addEventListener('click', () => openConversationInstructor(thread.id));
